...
Actions:
  ...
  drop-example remove one example that illustrates a different sense
    ...
    for path in sorted((ROOT / "data/families").glob("annotated-*.json")):
        ...
        for family in data["families"]:
            for member in family["members"]:
                key = (member["word"].replace(" ", "_"), member["synset"])
...
def main():
    ...
    for sense_id, d in decisions.items():
        ...
        if action == "tone":
            ...
            member["examples"] = [e for e in ex if e != d["example"]]
            if len(member["examples"]) == len(ex):
                print(f"  warning: example not found on {sense_id}: {d['example']!r}")
